...
if ($trackedLogs.Count -eq 0) {
    Write-Warning "No tracked output/*.log files found."
    exit 0
}

foreach ($relativePath in $trackedLogs) {
    Invoke-Git @("update-index", "--no-skip-worktree", "--", $relativePath)
}

Write-Host "Cleared skip-worktree for $($trackedLogs.Count) tracked output log file(s)."
Write-Host "No files were deleted or reverted. Local log modifications are now visible to git."
